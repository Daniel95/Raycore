...
void Update(const sf::Vector2i& mousePos, const sf::Vector2f& displaySize,
    ...
    if (s_windowHasFocus) {
        if (io.WantSetMousePos) { // Changed the name of the variable below (tempMousePos) to supress a warning of hiding a function parameter (name was already in use)
            sf::Vector2i tempMousePos(static_cast<int>(io.MousePos.x),
									  static_cast<int>(io.MousePos.y));
            sf::Mouse::setPosition(tempMousePos);
        } else {
