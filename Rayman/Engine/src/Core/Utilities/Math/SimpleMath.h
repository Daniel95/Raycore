...
namespace DirectX {
	namespace SimpleMath {
		...
		struct Rectangle {
			...
			operator RECT() {
				RECT rct; rct.left = x; rct.top = y; rct.right = (x + width); rct.bottom = (y + height); return rct;
			}
			...
			Rectangle& operator=(_In_ const RECT& rct) {
				x = rct.left; y = rct.top; width = (rct.right - rct.left); height = (rct.bottom - rct.top); return *this;
			}
		...
		struct Vector3 : public XMFLOAT3 {
			...
			explicit Vector3(const XMVECTORF32& F) {
				this->x = F.f[0]; this->y = F.f[1]; this->z = F.f[2];
			}
			...
			Vector3& operator= (const XMVECTORF32& F) {
				x = F.f[0]; y = F.f[1]; z = F.f[2]; return *this;
			}
		...
		struct Vector4 : public XMFLOAT4 {
			...
			explicit Vector4(const XMVECTORF32& F) {
				this->x = F.f[0]; this->y = F.f[1]; this->z = F.f[2]; this->w = F.f[3];
			}
			...
			Vector4& operator= (const XMVECTORF32& F) {
				x = F.f[0]; y = F.f[1]; z = F.f[2]; w = F.f[3]; return *this;
			}
