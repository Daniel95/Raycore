...
public:
	...
	template<typename T>
	void SetGraphicsDynamicStructuredBufferPointer(uint32_t slot, const std::vector<T*>& bufferData)
	{
		SetGraphicsDynamicStructuredBuffer(slot, bufferData.size(), sizeof(T), (void*) *bufferData.data());
	}
    /**
     * Set viewports.
     */
