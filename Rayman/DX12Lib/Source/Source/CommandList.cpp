...
void CommandList::LoadTextureFromFile( DX12Texture& texture, const std::wstring& fileName, TextureUsage textureUsage )
{
    auto device = DX12Graphics::Get().GetDevice();
...
void CommandList::GenerateMips( DX12Texture& texture )
{
    if ( m_d3d12CommandListType == D3D12_COMMAND_LIST_TYPE_COPY )
    {
        if ( !m_GenerateMipsCommandList )
        {
            m_GenerateMipsCommandList = DX12Graphics::Get().GetCommandQueue( D3D12_COMMAND_LIST_TYPE_COMPUTE )->GetCommandList();
        }
...
void CommandList::GenerateMips_UAV( DX12Texture& texture )
{
...
void CommandList::GenerateMips_BGR( DX12Texture& texture )
{
    auto device = DX12Graphics::Get().GetDevice();
...
void CommandList::GenerateMips_sRGB( DX12Texture& texture )
{
    auto device = DX12Graphics::Get().GetDevice();
    ...
    // Create a heap to alias the resource. This is used to copy the resource without 
    // failing GPU validation.
    auto allocationInfo = device->GetResourceAllocationInfo(0, 1, &copyDesc);
    auto bufferSize = GetRequiredIntermediateSize(resource.Get(), 0, resourceDesc.MipLevels);
...
void CommandList::ClearTexture( const DX12Texture& texture, const float clearColor[4])
{
...
void CommandList::ClearDepthStencilTexture( const DX12Texture& texture, D3D12_CLEAR_FLAGS clearFlags, float depth, uint8_t stencil)
{
...
void CommandList::CopyTextureSubresource( DX12Texture& texture, uint32_t firstSubresource, uint32_t numSubresources, D3D12_SUBRESOURCE_DATA* subresourceData )
{
    auto device = DX12Graphics::Get().GetDevice();
