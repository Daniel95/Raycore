...
void DX12Mesh::Draw(CommandList& commandList, uint32_t instanceCount, uint32_t firstInstance)
{
...
std::unique_ptr<DX12Mesh> DX12Mesh::CreateSphere(CommandList& commandList, float diameter, size_t tessellation, bool rhcoords)
{
    ...
    std::unique_ptr<DX12Mesh> mesh(new DX12Mesh());
...
std::unique_ptr<DX12Mesh> DX12Mesh::CreateCube(CommandList& commandList, float size, bool rhcoords)
{
...
std::unique_ptr<DX12Mesh> DX12Mesh::CreateCone(CommandList& commandList, float diameter, float height, size_t tessellation, bool rhcoords)
{
    VertexCollection vertices;
    IndexCollection indices;
...
std::unique_ptr<DX12Mesh> DX12Mesh::CreateTorus(CommandList& commandList, float diameter, float thickness, size_t tessellation, bool rhcoords)
{
    VertexCollection vertices;
    IndexCollection indices;
...
std::unique_ptr<DX12Mesh> DX12Mesh::CreatePlane(CommandList& commandList, float width, float height, bool rhcoords)
{
    //VertexCollection vertices = 
    //{
    //    { XMFLOAT3(-0.5f * width, -0.5f * height, 0.0f), XMFLOAT3(0.0f, 0.0f, -1.0f), XMFLOAT2(0.0f, 0.0f) }, // 0
...
std::unique_ptr<DX12Mesh> DX12Mesh::CreatePlaneTextured(CommandList& commandList, float startX, float startY, float widthT, float heightT, bool rhcoords)
{
	float width = 1.0f;
	float height = 1.0f;
	...
	VertexCollection vertices =
	{
		{ XMFLOAT3(-0.5f * width, -0.5f * height, 0.0f), XMFLOAT3(0.0f, 0.0f, -1.0f), XMFLOAT2(startX, startY) }, // 0
		{ XMFLOAT3(0.5f * width, -0.5f * height, 0.0f), XMFLOAT3(0.0f, 0.0f, -1.0f), XMFLOAT2(widthT,startY) }, // 1
		{ XMFLOAT3(0.5f * width, 0.5f * height, 0.0f), XMFLOAT3(0.0f, 0.0f, -1.0f), XMFLOAT2(widthT, heightT) }, // 2
		{ XMFLOAT3(-0.5f * width, 0.5f * height, 0.0f), XMFLOAT3(0.0f, 0.0f, -1.0f), XMFLOAT2(startX, heightT) }  // 3
	};
...
std::unique_ptr<DX12Mesh> DX12Mesh::CreatePlaneByTextureIndex(CommandList& commandList, int index, int texWidth, int texHeight, int tWidth, int tHeight, int tilesPerRow, int margin)
{
	float leftX;
	float rightX;
	float topY;
	...
	int indexX = index % tilesPerRow;
	int indexY = static_cast<int>(floor(index / tilesPerRow));

	leftX = static_cast<float>(indexX * (tWidth + margin));
	rightX = leftX + tWidth;
	topY =  static_cast<float>(indexY * (tHeight + margin));
	bottomY = topY + tHeight;
	...
	bottomY /= texHeight;
//
	return DX12Mesh::CreatePlaneTextured(commandList, leftX, topY, rightX, bottomY);
	//return Mesh::CreatePlaneTextured(commandList, 0,0,1,1);
}
...
void DX12Mesh::Initialize(CommandList& commandList, VertexCollection& vertices, IndexCollection& indices, bool rhcoords)
{
