...
Window::Window(HWND hWnd, const std::wstring& windowName, int clientWidth, int clientHeight, bool vSync)
    ...
    , m_FrameValues{0}
{
    DX12Graphics& app = DX12Graphics::Get();
...
void Window::RegisterCallbacks(std::shared_ptr<DX12Application> pGame)
{
    m_pGame = pGame;
...
void Window::OnFixedUpdate(FixedUpdateEventArgs& e)
{
	auto& lag = m_UpdateClock.GetLag();

	while (lag >= gc_TimestepNano)
	{
		m_UpdateClock.UpdateLag();

		if (auto pGame = m_pGame.lock())
		{
			FixedUpdateEventArgs fixedUpdateEventArgs(static_cast<float>(m_UpdateClock.GetFixedTimeStepSeconds()));
			pGame->OnFixedUpdate(fixedUpdateEventArgs);
		}
	}
}
...
void Window::OnResize(ResizeEventArgs& e)
{
    ...
    if (m_ClientWidth != e.Width || m_ClientHeight != e.Height)
    {
        ...
        DX12Graphics::Get().Flush();

        // Release all references to back buffer textures.
        m_RenderTarget.AttachTexture( Color0, DX12Texture() );
        for (int i = 0; i < BufferCount; ++i)
        {
...
UINT Window::Present( const DX12Texture& texture )
{
    ...
    m_FrameValues[m_CurrentBackBufferIndex] = DX12Graphics::GetFrameCount();

    m_CurrentBackBufferIndex = m_dxgiSwapChain->GetCurrentBackBufferIndex();

    commandQueue->WaitForFenceValue( m_FenceValues[m_CurrentBackBufferIndex] );

    DX12Graphics::Get().ReleaseStaleDescriptors( m_FrameValues[m_CurrentBackBufferIndex] );
