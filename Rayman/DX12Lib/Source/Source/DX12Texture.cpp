...
D3D12_CPU_DESCRIPTOR_HANDLE DX12Texture::GetShaderResourceView() const
{
    return m_ShaderResourceView.GetDescriptorHandle();
}

D3D12_CPU_DESCRIPTOR_HANDLE DX12Texture::GetUnorderedAccessView(uint32_t subresource) const
{
    return m_UnorderedAccessViews.GetDescriptorHandle( subresource );
}

D3D12_CPU_DESCRIPTOR_HANDLE DX12Texture::GetUnorderedAccessView(uint32_t mipSlice, uint32_t arraySlice, uint32_t planeSlice) const
{
    CD3DX12_RESOURCE_DESC desc(m_d3d12Resource->GetDesc());
