...
static DX12Graphics* gs_pSingelton = nullptr;
static WindowMap gs_Windows;
static WindowNameMap gs_WindowByName;

uint64_t DX12Graphics::ms_FrameCount = 0;
...
DX12Graphics::DX12Graphics(HINSTANCE hInst)
    : m_hInstance(hInst)
...
void DX12Graphics::Create(HINSTANCE hInst)
{
    if (!gs_pSingelton)
    {
        gs_pSingelton = new DX12Graphics(hInst);
...
Microsoft::WRL::ComPtr<IDXGIAdapter4> DX12Graphics::GetAdapter(bool bUseWarp)
{
...
Microsoft::WRL::ComPtr<ID3D12Device2> DX12Graphics::CreateDevice(Microsoft::WRL::ComPtr<IDXGIAdapter4> adapter)
{
...
bool DX12Graphics::CheckTearingSupport()
{
...
bool DX12Graphics::IsTearingSupported() const
{
    return m_TearingSupported;
}

DXGI_SAMPLE_DESC DX12Graphics::GetMultisampleQualityLevels( DXGI_FORMAT format, UINT numSamples, D3D12_MULTISAMPLE_QUALITY_LEVEL_FLAGS flags ) const
{
...
std::shared_ptr<Window> DX12Graphics::CreateRenderWindow(const std::wstring& windowName, int clientWidth, int clientHeight, bool vSync )
{
...
void DX12Graphics::DestroyWindow(std::shared_ptr<Window> window)
{
    if (window) window->Destroy();
}

void DX12Graphics::DestroyWindow(const std::wstring& windowName)
{
...
std::shared_ptr<Window> DX12Graphics::GetWindowByName(const std::wstring& windowName)
{
...
int DX12Graphics::Run(std::shared_ptr<DX12Application> pGame)
{
...
void DX12Graphics::Quit(int exitCode)
{
    PostQuitMessage(exitCode);
}

Microsoft::WRL::ComPtr<ID3D12Device2> DX12Graphics::GetDevice() const
{
    return m_d3d12Device;
}

std::shared_ptr<CommandQueue> DX12Graphics::GetCommandQueue(D3D12_COMMAND_LIST_TYPE type) const
{
...
void DX12Graphics::ReleaseStaleDescriptors( uint64_t finishedFrame )
{
    for ( int i = 0; i < D3D12_DESCRIPTOR_HEAP_TYPE_NUM_TYPES; ++i )
    {
...
Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> DX12Graphics::CreateDescriptorHeap(UINT numDescriptors, D3D12_DESCRIPTOR_HEAP_TYPE type)
{
...
UINT DX12Graphics::GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE type) const
{
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if ( pWindow )
    {
        ...
        case WM_PAINT:
        {
            ++DX12Graphics::ms_FrameCount;

			//Fixed update loop.
			FixedUpdateEventArgs fixedUpdateEventArgs(0.0f);
			pWindow->OnFixedUpdate(fixedUpdateEventArgs);

            // Delta time will be filled in by the Window.
            UpdateEventArgs updateEventArgs(0.0f, 0.0f, DX12Graphics::ms_FrameCount);
            pWindow->OnUpdate(updateEventArgs);

            RenderEventArgs renderEventArgs(0.0f, 0.0f, DX12Graphics::ms_FrameCount);
            // Delta time will be filled in by the Window.
