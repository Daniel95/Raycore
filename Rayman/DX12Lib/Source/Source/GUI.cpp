...
bool GUI::Initialize(std::shared_ptr<Window> a_Window)
{
	m_Window = a_Window;
	...
	m_FontTexture = std::make_unique<DX12Texture>(fontTextureDesc);
...
void GUI::Render(std::shared_ptr<CommandList> a_CommandList, const RenderTarget& a_RenderTarget)
{
	ImGui::SetCurrentContext(m_pImGuiCtx);
	ImGui::Render();
	...
	a_CommandList->SetGraphicsRootSignature(*m_RootSignature);
	a_CommandList->SetPipelineState(m_PipelineState);
	a_CommandList->SetRenderTarget(a_RenderTarget);
...
//    DirectX::XMMATRIX projectionMatrix = DirectX::XMMatrixOrthographicRH( drawData->DisplaySize.x, drawData->DisplaySize.y, 0.0f, 1.0f );
	...
	a_CommandList->SetGraphics32BitConstants(RootParameters::MatrixCB, mvp);
	a_CommandList->SetShaderResourceView(RootParameters::FontTexture, 0, *m_FontTexture, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
	...
	a_CommandList->SetViewport(viewport);
	a_CommandList->SetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	...
	a_CommandList->FlushResourceBarriers();

	for (int i = 0; i < drawData->CmdListsCount; ++i)
	{
		const ImDrawList* drawList = drawData->CmdLists[i];

		a_CommandList->SetDynamicVertexBuffer(0, drawList->VtxBuffer.size(), sizeof(ImDrawVert), drawList->VtxBuffer.Data);
		a_CommandList->SetDynamicIndexBuffer(drawList->IdxBuffer.size(), indexFormat, drawList->IdxBuffer.Data);
		...
		for (int j = 0; j < drawList->CmdBuffer.size(); ++j)
		{
			...
			else
			{
				...
				{
					a_CommandList->SetScissorRect(scissorRect);
					a_CommandList->DrawIndexed(drawCmd.ElemCount, 1, indexOffset);
				}
			}
